...
export const protestExtraController = {
  addAttachment: async (request: Request, response: Response) => response.status(201).json(await service.addAttachment(request.params.id, request.user!.id, request.body, request.file)),
