...
export async function createUser(data: { name: string; email: string; password: string; role: "ADMIN" | "EMPLOYEE"; active: boolean }) {
  const { password, ...userData } = data;
  const passwordHash = await bcrypt.hash(data.password, 10);
  return prisma.user.create({ data: { ...userData, passwordHash }, select });
}

export async function updateUser(id: string, data: Partial<{ name: string; email: string; password: string; role: "ADMIN" | "EMPLOYEE"; active: boolean }>) {
  const { password, ...userData } = data;
